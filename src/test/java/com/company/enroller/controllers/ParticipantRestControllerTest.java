...
public class ParticipantRestControllerTest {
	...
	public void getParticipant() throws Exception {
		...
		Collection<Participant> participantSingleton = singletonList(participant);
		given(participantService.getAll()).willReturn(participantSingleton);
		
		mvc.perform(get("/participants").contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
			.andExpect(jsonPath("$[0].login", is(participant.getLogin()))).andExpect(jsonPath("$[0].password", is(participant.getPassword())));
